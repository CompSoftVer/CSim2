...
either write \<open>CALL Square(\<acute>I,\<acute>R)\<close> or @{term "\<acute>I :== CALL\<^sub>s
Square(\<acute>R)"} for the procedure call. The internal term is the
following:
\<close>

(*<*) declare [[hoare_use_call_tr' = false]] (*>*)
text \<open>\small @{term [display] "CALL\<^sub>s Square(\<acute>I,\<acute>R)"}\<close>
