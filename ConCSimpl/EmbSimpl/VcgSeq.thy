...
keywords "procedures\<^sub>s" "hoarestate\<^sub>s" :: thy_defn
begin
...
syntax
  "_raise_s":: "'c \<Rightarrow> 'c \<Rightarrow> ('a,'b,'f) com"       ("(RAISE _ :==\<^sub>s/ _)" [30, 30] 23)
  "_seq_s"::"('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com" ("_;;\<^sub>s/ _" [20, 21] 20)
  "_guards_s"        :: "grds \<Rightarrow> ('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com"
                                            ("(_/\<longmapsto>\<^sub>s _)" [60, 21] 23)
  "_Assign_s"      :: "'b => 'b => ('a,'p,'f) com"    ("(_ :==\<^sub>s/ _)" [30, 30] 23)
  "_Init_s"        :: "ident \<Rightarrow> 'c \<Rightarrow> 'b \<Rightarrow> ('a,'p,'f) com"
                                             ("(\<acute>_ :==\<^sub>s\<^bsub>_\<^esub>/ _)" [30,1000, 30] 23)
  "_GuardedAssign_s":: "'b => 'b => ('a,'p,'f) com"    ("(_ :==\<^sub>g\<^sub>s/ _)" [30, 30] 23)
  
  "_New_s"         :: "['a, 'b, newinits] \<Rightarrow> ('a,'b,'f) com"
                                            ("(_ :==\<^sub>s/(2 NEW _/ [_]))" [30, 65, 0] 23)
  "_GuardedNew_s"  :: "['a, 'b, newinits] \<Rightarrow> ('a,'b,'f) com"
                                            ("(_ :==\<^sub>g\<^sub>s/(2 NEW _/ [_]))" [30, 65, 0] 23)
  "_NNew_s"         :: "['a, 'b, newinits] \<Rightarrow> ('a,'b,'f) com"
                                            ("(_ :==\<^sub>s/(2 NNEW _/ [_]))" [30, 65, 0] 23)
  "_GuardedNNew_s"  :: "['a, 'b, newinits] \<Rightarrow> ('a,'b,'f) com"
                                            ("(_ :==\<^sub>g\<^sub>s/(2 NNEW _/ [_]))" [30, 65, 0] 23)

  "_Cond_s"        :: "'a bexp => ('a,'p,'f) com => ('a,'p,'f) com => ('a,'p,'f) com"
        ("(0IF\<^sub>s (_)/ (2THEN/ _)/ (2ELSE _)/ FI)" [0, 0, 0] 71)
  "_Cond_no_else_s":: "'a bexp => ('a,'p,'f) com => ('a,'p,'f) com"
        ("(0IF\<^sub>s (_)/ (2THEN/ _)/ FI)" [0, 0] 71)
  "_GuardedCond_s" :: "'a bexp => ('a,'p,'f) com => ('a,'p,'f) com => ('a,'p,'f) com"
        ("(0IF\<^sub>g\<^sub>s (_)/ (2THEN _)/ (2ELSE _)/ FI)" [0, 0, 0] 71)
  "_GuardedCond_no_else_s":: "'a bexp => ('a,'p,'f) com => ('a,'p,'f) com"
  ...
  "_GuardedWhile_inv_var_s"::
       "'a bexp => 'a assn  \<Rightarrow> ('a \<times> 'a) set \<Rightarrow> bdy_s \<Rightarrow> ('a,'p,'f) com"
        ("(0WHILE\<^sub>g\<^sub>s (_)/ INV (_)/ VAR (_) /_)"  [25, 0, 0, 81] 71)
  "_While_inv_s"   :: "'a bexp => 'a assn => bdy_s => ('a,'p,'f) com"
        ("(0WHILE\<^sub>s (_)/ INV (_) /_)"  [25, 0, 81] 71)
  "_GuardedWhile_inv_s"   :: "'a bexp => 'a assn => ('a,'p,'f) com => ('a,'p,'f) com"
        ("(0WHILE\<^sub>g\<^sub>s (_)/ INV (_) /_)"  [25, 0, 81] 71)
  "_While_s"       :: "'a bexp => bdy_s => ('a,'p,'f) com"
        ("(0WHILE\<^sub>s (_) /_)"  [25, 81] 71)
  "_GuardedWhile_s"       :: "'a bexp => bdy_s => ('a,'p,'f) com"
        ("(0WHILE\<^sub>g\<^sub>s (_) /_)"  [25, 81] 71)
  "_While_guard_s"       :: "grds => 'a bexp => bdy_s => ('a,'p,'f) com"
        ("(0WHILE\<^sub>s (_/\<longmapsto> (1_)) /_)"  [1000,25,81] 71)
  "_While_guard_inv_s":: "grds \<Rightarrow>'a bexp\<Rightarrow>'a assn\<Rightarrow>bdy_s \<Rightarrow> ('a,'p,'f) com"
  ...
  "_While_guard_inv_var_s":: "grds \<Rightarrow>'a bexp\<Rightarrow>'a assn\<Rightarrow>('a\<times>'a) set
                             \<Rightarrow>bdy_s \<Rightarrow> ('a,'p,'f) com"
  ...
  "_Try_Catch_s":: "('a,'p,'f) com \<Rightarrow>('a,'p,'f) com \<Rightarrow> ('a,'p,'f) com"
        ("(0TRY\<^sub>s (_)/ (2CATCH _)/ END)"  [0,0] 71)

  "_DoPre_s" :: "('a,'p,'f) com \<Rightarrow> ('a,'p,'f) com"
  "_Do_s" :: "('a,'p,'f) com \<Rightarrow> bdy_s" ("(2DO\<^sub>s/ (_)) /OD" [0] 1000)
  "_Lab_s":: "'a bexp \<Rightarrow> ('a,'p,'f) com \<Rightarrow> bdy_s"
            ("_\<bullet>\<^sub>s/_" [1000,71] 81)
  "":: "bdy_s \<Rightarrow> ('a,'p,'f) com" ("_")
  "_Spec_s":: "pttrn \<Rightarrow> 's set \<Rightarrow>  ('s,'p,'f) com \<Rightarrow> 's set \<Rightarrow> 's set \<Rightarrow> ('s,'p,'f) com"
            ("(ANNO\<^sub>s _. _/ (_)/ _,/_)" [0,1000,20,1000,1000] 60)
  "_SpecNoAbrupt_s":: "pttrn \<Rightarrow> 's set \<Rightarrow>  ('s,'p,'f) com \<Rightarrow> 's set \<Rightarrow> ('s,'p,'f) com"
            ("(ANNO\<^sub>s _. _/ (_)/ _)" [0,1000,20,1000] 60)
  "_LemAnno_s":: "'n \<Rightarrow> ('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com"
              ("(0 LEMMA\<^sub>s (_)/ _ END)" [1000,0] 71)
  
  "_Loc_s":: "[locinits,('s,'p,'f) com] \<Rightarrow> ('s,'p,'f) com"
                                         ("(2 LOC\<^sub>s _;;/ (_) COL)" [0,0] 71)
  "_Switch_s":: "('s \<Rightarrow> 'v) \<Rightarrow> switchcases_s \<Rightarrow> ('s,'p,'f) com"
              ("(0 SWITCH\<^sub>s (_)/ _ END)" [22,0] 71)
  "_switchcase_s":: "'v set \<Rightarrow> ('s,'p,'f) com \<Rightarrow> switchcase_s" ("_\<Rightarrow>\<^sub>s/ _" )
  ...
  "_switchcasesCons_s":: "switchcase_s \<Rightarrow> switchcases_s \<Rightarrow> switchcases_s"
                       ("_/ |\<^sub>s _")
  "_Basic_s":: "basicblock \<Rightarrow> ('s,'p,'f) com" ("(0BASIC\<^sub>s/ (_)/ END)" [22] 71)
...
syntax (ASCII) 
  "_While_guard_s"       :: "grds => 'a bexp => bdy_s \<Rightarrow> ('a,'p,'f) com"
        ("(0WHILE\<^sub>s (_|-> /_) /_)"  [0,0,1000] 71)
  "_While_guard_inv_s":: "grds\<Rightarrow>'a bexp\<Rightarrow>'a assn\<Rightarrow>bdy_s \<Rightarrow> ('a,'p,'f) com"
        ("(0WHILE\<^sub>s (_|-> /_) INV (_) /_)"  [0,0,0,1000] 71)
  "_guards_s" :: "grds \<Rightarrow> ('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com" ("(_|->\<^sub>s_ )" [60, 21] 23)
...
syntax "_Call_s" :: "'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)" ("CALL\<^sub>s __" [1000,1000] 21)
       "_GuardedCall_s" :: "'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)" ("CALL\<^sub>g\<^sub>s __" [1000,1000] 21)
       "_CallAss_s":: "'a \<Rightarrow> 'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)"
             ("_ :== CALL\<^sub>s __" [30,1000,1000] 21)
       "_Proc_s" :: "'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)" ("PROC\<^sub>s __" 21)
       "_ProcAss_s":: "'a \<Rightarrow> 'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)"
             ("_ :== PROC\<^sub>s __" [30,1000,1000] 21)
       "_GuardedCallAss_s":: "'a \<Rightarrow> 'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)"
             ("_ :== CALL\<^sub>g\<^sub>s __" [30,1000,1000] 21)
       "_DynCall_s" :: "'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)" ("DYNCALL\<^sub>s __" [1000,1000] 21)
       "_GuardedDynCall_s" :: "'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)" ("DYNCALL\<^sub>g\<^sub>s __" [1000,1000] 21)
       "_DynCallAss_s":: "'a \<Rightarrow> 'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)"
             ("_ :== DYNCALL\<^sub>s __" [30,1000,1000] 21)
       "_GuardedDynCallAss_s":: "'a \<Rightarrow> 'p \<Rightarrow> actuals \<Rightarrow> (('a,string,'f) com)"
             ("_ :== DYNCALL\<^sub>g\<^sub>s __" [30,1000,1000] 21)

       "_Bind_s":: "['s \<Rightarrow> 'v, idt, 'v \<Rightarrow> ('s,'p,'f) com] \<Rightarrow> ('s,'p,'f) com"
                      ("_ \<ggreater>\<^sub>s _./ _" [22,1000,21] 21)
       "_bseq_s"::"('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com \<Rightarrow> ('s,'p,'f) com"
           ("_\<ggreater>\<^sub>s/ _" [22, 21] 21)
       "_FCall_s" :: "['p,actuals,idt,(('a,string,'f) com)]\<Rightarrow> (('a,string,'f) com)"
