...
record globals_state = x_'::"nat" z_'::"nat"
record locals_state = N_' :: "nat" yy_' :: "nat"
...
lemma "\<Gamma>\<turnstile> \<lbrace>\<acute>N = 5\<rbrace> \<acute>N :==\<^sub>s 2 * \<acute>N \<lbrace>\<acute>N = 10\<rbrace>"
  apply vcg
  txt {* @{subgoals} *}
  apply simp
  txt {* @{subgoals} *}
...
record locals_1 = ppppp_' :: "ref" pp_' :: "nat" p_' :: "ref"

abbreviation "ppp_' \<equiv>ppppp_'"
abbreviation "ppp_'_update \<equiv>ppppp_'_update"

term "\<acute>ppp :== \<acute>ppp"

term "\<acute>p :== NEW sz [\<acute>cont:==0,\<acute>next:== Null]"
