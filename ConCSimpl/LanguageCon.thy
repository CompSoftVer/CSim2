...
| "sequential (Fault f) = Language.Skip" 
| "sequential Stuck = Language.Skip"
...
| "noawaits (Fault f) = False"
| "noawaits Stuck = False"
...
definition
  specAnno::  "('a \<Rightarrow> ('s, 'p, 'f, 'e) com) \<Rightarrow> 
              ('a \<Rightarrow> ('s\<times>'l) assn) \<Rightarrow> ('a \<Rightarrow> ('c\<times>'c) assn) \<Rightarrow> 
              ('a \<Rightarrow> ('c\<times>'c) assn) \<Rightarrow> 
              ('a \<Rightarrow> ('s\<times>'l) assn) \<Rightarrow> ('a \<Rightarrow> ('s\<times>'l) assn) \<Rightarrow>
              ('s, 'p, 'f, 'e) com"
  where "specAnno c P R G Q A = (c undefined)"
...
"flattenc (Fault f) = [Fault f]" |
"flattenc (Stuck) = [Stuck]" 
...
"normalize (Fault f) = Fault f" |
"normalize Stuck = Stuck"
...
"normalizec (Fault f) = Fault f" |
"normalizec Stuck = Stuck" 
...
lemma flatten_specAnno [simp]:
  "flatten (specAnno c P R G Q A) = flatten (c undefined)"
...
lemma normalize_specAnno [simp]:
  "normalize (specAnno c P R G Q A) = specAnno (\<lambda>s. normalize (c undefined)) P R G Q A"
  by (simp add: specAnno_def)
...
lemma flattenc_specAnno [simp]:
  "flattenc (specAnno c P R G Q A) = flattenc (c undefined)"
  by (simp add: specAnno_def)
...
lemma normalizec_specAnno:
  "normalizec (specAnno c P R G Q A) = specAnno (\<lambda>s. normalizec (c undefined)) P R G Q A"
  by (simp add: specAnno_def)
...
"strip_guards F (Fault f) = Fault f" |
"strip_guards F Stuck = Stuck"
...
lemma strip_guards_specAnno [simp]:
  "strip_guards F (specAnno c P R G Q A) = 
    specAnno (\<lambda>s. strip_guards F (c undefined)) P R G Q A"
  by (simp add: specAnno_def)
...
"mark_guards f Stuck = Stuck" |
"mark_guards f (Fault f') = Fault f"
...
lemma mark_guards_specAnno [simp]:
  "mark_guards f (specAnno c P R G Q A) = 
    specAnno (\<lambda>s. mark_guards f (c undefined)) P R G Q A"
  by (simp add: specAnno_def)
...
lemma is_Guard_basic_simps [simp]:
 ...
 "is_Guard (Fault f) = False"
 "is_Guard Stuck = False"
...
"merge_guards (Fault f) = Fault f" |
"merge_guards Stuck = Stuck" |
...
lemma merge_guards_res_Fault: "merge_guards c = Fault f \<Longrightarrow> \<exists>f'. c = Fault f"
  by (cases c) (auto split: com.splits if_split_asm simp add: is_Guard_def Let_def)

lemma merge_guards_res_Stuck: "merge_guards c = Stuck \<Longrightarrow>  c = Stuck"
  by (cases c) (auto split: com.splits if_split_asm simp add: is_Guard_def Let_def)
...
lemmas merge_guards_res_simps = merge_guards_res_Skip merge_guards_res_Basic 
 ...
 merge_guards_res_Guard merge_guards_res_Await merge_guards_res_Fault merge_guards_res_Stuck
...
lemma merge_guards_specAnno [simp]:
  "merge_guards (specAnno c P R G Q A) = 
    specAnno (\<lambda>s. merge_guards (c undefined)) P R G Q A"
  by (simp add: specAnno_def)
...
"nothrows Stuck = True" |
"nothrows (Fault f) = True" 

primrec noFaults:: "('s, 'p, 'f, 'e) com \<Rightarrow> bool"
where
"noFaults Skip = True" |
"noFaults (Basic f e) = True" |
"noFaults (Spec r e) = True" |
"noFaults (Seq c\<^sub>1 c\<^sub>2)  = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noFaults (Cond b c\<^sub>1 c\<^sub>2) = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noFaults (While b c) = nothrows c" |
"noFaults (Call p) = True" |
"noFaults (DynCom c) = (\<forall>s. nothrows (c s))" |
"noFaults (Guard f g c) = nothrows c" |
"noFaults Throw = True" |
"noFaults (Catch c\<^sub>1 c\<^sub>2) = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noFaults (Await b cn e) = Language.nothrows cn" |
"noFaults Stuck = True" |
"noFaults (Fault f) = False" 

primrec noStucks:: "('s, 'p, 'f, 'e) com \<Rightarrow> bool"
where
"noStucks Skip = True" |
"noStucks (Basic f e) = True" |
"noStucks (Spec r e) = True" |
"noStucks (Seq c\<^sub>1 c\<^sub>2)  = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noStucks (Cond b c\<^sub>1 c\<^sub>2) = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noStucks (While b c) = nothrows c" |
"noStucks (Call p) = True" |
"noStucks (DynCom c) = (\<forall>s. nothrows (c s))" |
"noStucks (Guard f g c) = nothrows c" |
"noStucks Throw = True" |
"noStucks (Catch c\<^sub>1 c\<^sub>2) = (nothrows c\<^sub>1 \<and> nothrows c\<^sub>2)" |
"noStucks (Await b cn e) = Language.nothrows cn" |
"noStucks Stuck = False" |
"noStucks (Fault f) = True"
...
"(Fault f1 \<inter>\<^sub>g\<^sub>s Fault f2) = (if (f1=f2) then Some (Fault f1) else None)"

"(Stuck \<inter>\<^sub>g\<^sub>s Stuck) = Some Stuck"
...
lemma inter_guards_Fault:
 "(Fault f \<inter>\<^sub>g\<^sub>s c2 ) = Some c = (\<exists>f1. c2 = Fault f \<and> c = Fault f)"
  by (cases c2) (auto split: option.splits)

lemma inter_guards_Stuck:
 "(Stuck \<inter>\<^sub>g\<^sub>s c2 ) = Some c = (c2 = Stuck \<and> c = Stuck)"
by (cases c2) (auto split: option.splits)
...
lemmas inter_guards_simps = inter_guards_Skip inter_guards_Basic inter_guards_Spec
  ...
  inter_guards_Catch inter_guards_Await inter_guards_Fault inter_guards_Stuck
...
"(Stuck \<subseteq>\<^sub>g\<^sub>s Stuck) = True"
"(Fault f1 \<subseteq>\<^sub>g\<^sub>s Fault f2) = (f1 = f2)"
...
lemma subseteq_guards_Fault:
  "c \<subseteq>\<^sub>g\<^sub>s Fault f1 \<Longrightarrow> \<exists>f1'. c=Fault f1'"
  by (cases c) (auto)

lemma subseteq_guards_Stuck:
  "c \<subseteq>\<^sub>g\<^sub>s Stuck \<Longrightarrow> c = Stuck"
  by (cases c) (auto)
...
lemmas subseteq_guardsD = subseteq_guards_Skip subseteq_guards_Basic
 ...
 subseteq_guards_Throw subseteq_guards_Catch subseteq_guards_Await subseteq_guards_Fault
subseteq_guards_Stuck
