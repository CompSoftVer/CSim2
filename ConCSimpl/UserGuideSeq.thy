...
lemma (in vars) "\<Gamma>\<turnstile> \<lbrace>\<acute>N = 5\<rbrace> \<acute>N :==\<^sub>s 2 * \<acute>N \<lbrace>\<acute>N = 10\<rbrace>"
  apply vcg
...
lemma (in vars)
  "\<Gamma>,{}\<turnstile> \<lbrace>\<acute>M = 0 \<and> \<acute>S = 0\<rbrace>
          WHILE\<^sub>s \<acute>M \<noteq> a
          INV \<lbrace>\<acute>S = \<acute>M * b\<rbrace>
          DO\<^sub>s \<acute>S :==\<^sub>s \<acute>S + b;;\<^sub>s \<acute>M :==\<^sub>s \<acute>M + 1 OD
...
either write  or @{term "Language.call (\<lambda>s. s\<lparr>locals := locals s\<langle>N_'Square_' := locals s\<cdot>I_'Square_'\<rangle>\<rparr>)  Square_'proc (\<lambda>s t. s\<lparr>globals := globals t\<rparr>)"} for the procedure call. The internal term is the
following:                   
\<close>

(*<*) declare [[HoareCon.hoare_use_call_tr' = false]] (*>*)
...
shows "\<forall>n. \<Gamma>\<turnstile>\<lbrace>\<acute>N = n\<rbrace>  \<acute>R :== PROC\<^sub>s Square(\<acute>N) \<lbrace>\<acute>R = n * n\<rbrace>"
...
lemma (in Square_impl)
  shows "\<Gamma>\<turnstile>\<lbrace>\<acute>I = 2\<rbrace> \<acute>R :== CALL\<^sub>s Square(\<acute>I) \<lbrace>\<acute>R = 4\<rbrace>"
...
instantiated with the specification
  ...
  @{term "\<forall>n. \<Gamma>\<turnstile> \<lbrace>\<acute>I = n\<rbrace> \<acute>R :== CALL\<^sub>s Square(\<acute>I) \<lbrace>\<acute>R = n * n\<rbrace>"}.
...
procedures\<^sub>s
  Fac (N::nat | R::nat)
  "IF\<^sub>s \<acute>N = 0 THEN \<acute>R :==\<^sub>s 1
   ELSE \<acute>R :== CALL\<^sub>s Fac(\<acute>N - 1);;\<^sub>s
        \<acute>R :==\<^sub>s \<acute>N * \<acute>R
   FI"
...
lemma (in Fac_impl)
shows "\<forall>n. \<Gamma>\<turnstile> \<lbrace>\<acute>N = n\<rbrace> \<acute>R :== PROC\<^sub>s Fac(\<acute>N) \<lbrace>\<acute>R = fac n\<rbrace>"
...
procedures\<^sub>s (imports globals_heap)
  append(p :: ref, q::ref | p::ref)
    "IF\<^sub>s \<acute>p=Null THEN \<acute>p :==\<^sub>s \<acute>q
     ELSE \<acute>p\<rightarrow>\<acute>next :== CALL\<^sub>s append(\<acute>p\<rightarrow>\<acute>next,\<acute>q) FI"
...
We can study this effect on the translation of @{term "\<acute>p :== CALL\<^sub>s append(\<acute>p,\<acute>q)"}:
\<close>
(*<*)
declare [[hoare_use_call_tr' = false]]
(*>*)
text \<open>
@{term [display] "\<acute>p :== CALL\<^sub>s append(\<acute>p,\<acute>q)"}
\<close>
(*<*)
declare [[hoare_use_call_tr' = true]]
end
...
shows "\<forall>\<sigma> Ps Qs.
  \<Gamma>\<turnstile> \<lbrace>\<sigma>. List \<acute>p \<acute>next Ps \<and>  List \<acute>q \<acute>next Qs \<and> set Ps \<inter> set Qs = {}\<rbrace>
       \<acute>p :== PROC\<^sub>s append(\<acute>p,\<acute>q)
...
shows "\<Gamma>\<turnstile> \<lbrace>\<acute>cont=c\<rbrace> \<acute>p :== CALL\<^sub>s append(Null,Null) \<lbrace>\<acute>cont=c\<rbrace>"
proof -
...
lemma (in append_impl) append_modifies:
  shows "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^bsub>/UNIV\<^esub> {\<sigma>} \<acute>p :== PROC\<^sub>s append(\<acute>p,\<acute>q)
...
shows "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^sub>t {\<sigma>} \<acute>R :==  PROC\<^sub>s Fac(\<acute>N) \<lbrace>\<acute>R = fac \<^bsup>\<sigma>\<^esup>N\<rbrace>"
...
procedures\<^sub>s
  pedal(N::nat,M::nat)
  "IF\<^sub>s 0 < \<acute>N THEN
     IF\<^sub>s 0 < \<acute>M THEN
       CALL\<^sub>s coast(\<acute>N- 1,\<acute>M- 1) FI;;\<^sub>s
       CALL\<^sub>s pedal(\<acute>N- 1,\<acute>M)
     FI"
  and

  coast(N::nat,M::nat)
  "CALL\<^sub>s pedal(\<acute>N,\<acute>M);;\<^sub>s
   IF\<^sub>s 0 < \<acute>M THEN CALL\<^sub>s coast(\<acute>N,\<acute>M- 1) FI"
...
shows "(\<forall>\<sigma>. \<Gamma>\<turnstile>\<^sub>t {\<sigma>} PROC\<^sub>s pedal(\<acute>N,\<acute>M) UNIV) \<and>
         (\<forall>\<sigma>. \<Gamma>\<turnstile>\<^sub>t {\<sigma>} PROC\<^sub>s coast(\<acute>N,\<acute>M) UNIV)"
...
guarded command @{term "\<acute>M :==\<^sub>g\<^sub>s (\<acute>M + 1) div \<acute>N"}. Here @{term "in_range"} is
uninterpreted by now.
\<close>

lemma (in vars) "\<Gamma>\<turnstile>\<lbrace>True\<rbrace> \<acute>M :==\<^sub>g\<^sub>s (\<acute>M + 1) div \<acute>N \<lbrace>True\<rbrace>"
apply vcg
txt \<open>@{subgoals}\<close>
oops
...
procedures\<^sub>s (imports globals_heap)
  insert(r::ref,p::ref | p::ref)
    "IF\<^sub>s \<acute>r=Null THEN SKIP\<^sub>s
     ELSE IF\<^sub>s \<acute>p=Null THEN \<acute>p :==\<^sub>s \<acute>r;;\<^sub>s \<acute>p\<rightarrow>\<acute>next :==\<^sub>s Null
          ELSE IF\<^sub>s \<acute>r\<rightarrow>\<acute>cont \<le> \<acute>p\<rightarrow>\<acute>cont
               THEN \<acute>r\<rightarrow>\<acute>next :==\<^sub>s \<acute>p;;\<^sub>s \<acute>p:==\<^sub>s\<acute>r
               ELSE \<acute>p\<rightarrow>\<acute>next :== CALL\<^sub>s insert(\<acute>r,\<acute>p\<rightarrow>\<acute>next)
               FI
          FI
     FI"

lemma (in insert_impl) insert_modifies:
   "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^bsub>/UNIV\<^esub> {\<sigma>} \<acute>p :== PROC\<^sub>s insert(\<acute>r,\<acute>p)
        {t. t may_only_modify_globals \<sigma> in [next]}"
...
lemma (in insert_impl) insert_spec:
  ...
  \<lbrace>\<sigma>. List \<acute>p \<acute>next Ps \<and> sorted (\<le>) (map \<acute>cont Ps) \<and>
    ...
    \<acute>p :== PROC\<^sub>s insert(\<acute>r,\<acute>p)
...
procedures\<^sub>s (imports globals_heap)
  ...
  where r::ref q::ref in
    "\<acute>r:==\<^sub>sNull;;\<^sub>s
     WHILE\<^sub>s (\<acute>p \<noteq> Null) DO\<^sub>s
       \<acute>q :==\<^sub>s \<acute>p;;\<^sub>s
       \<acute>p :==\<^sub>s \<acute>p\<rightarrow>\<acute>next;;\<^sub>s
       \<acute>r :== CALL\<^sub>s insert(\<acute>q,\<acute>r)
     OD;;\<^sub>s
     \<acute>p:==\<^sub>s\<acute>r"

lemma (in insertSort_impl) insertSort_modifies:
  shows
   "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^bsub>/UNIV\<^esub> {\<sigma>} \<acute>p :== PROC\<^sub>s insertSort(\<acute>p)
    {t. t may_only_modify_globals \<sigma> in [next]}"
apply (hoare_rule HoarePartial.ProcRec1)
apply (vcg spec=modifies)
done
...
shows "\<forall>\<sigma> Ps.
  \<Gamma>\<turnstile> \<lbrace>\<sigma>. List \<acute>p \<acute>next Ps \<rbrace>
       \<acute>p :== PROC\<^sub>s insertSort(\<acute>p)
     \<lbrace>\<exists>Qs. List \<acute>p \<acute>next Qs \<and> sorted (\<le>) (map \<^bsup>\<sigma>\<^esup>cont Qs) \<and>
           set Qs = set Ps\<rbrace>"
apply (hoare_rule HoarePartial.ProcRec1)
apply (hoare_rule anno=
         "\<acute>r :==\<^sub>s Null;;\<^sub>s
         WHILE\<^sub>s \<acute>p \<noteq> Null
         INV \<lbrace>\<exists>Qs Rs. List \<acute>p \<acute>next Qs \<and> List \<acute>r \<acute>next Rs \<and>
          ...
          DO\<^sub>s \<acute>q :==\<^sub>s \<acute>p;;\<^sub>s \<acute>p :==\<^sub>s \<acute>p\<rightarrow>\<acute>next;;\<^sub>s \<acute>r :== CALL\<^sub>s insert(\<acute>q,\<acute>r) OD;;\<^sub>s
          \<acute>p :==\<^sub>s \<acute>r" in HoarePartial.annotateI)
apply vcg
...
lemma (in vars) "\<Gamma>\<turnstile> {\<sigma>}
            \<acute>I :==\<^sub>s \<acute>M;;\<^sub>s
            ANNO\<^sub>s \<tau>. \<lbrace>\<tau>. \<acute>I = \<^bsup>\<sigma>\<^esup>M\<rbrace>
                         \<acute>M :==\<^sub>s \<acute>N;;\<^sub>s \<acute>N :==\<^sub>s \<acute>I
...
lemma (in vars)
  "\<Gamma>\<turnstile> {\<sigma>}
   \<acute>I :==\<^sub>s \<acute>M;;\<^sub>s
   ANNO\<^sub>s (n,i,m). \<lbrace>\<acute>I = \<^bsup>\<sigma>\<^esup>M \<and> \<acute>N=n \<and> \<acute>I=i \<and> \<acute>M=m\<rbrace>
     \<acute>M :==\<^sub>s \<acute>N;;\<^sub>s \<acute>N :==\<^sub>s \<acute>I
...
lemma (in vars) foo_lemma:
  "\<forall>n m. \<Gamma>\<turnstile> \<lbrace>\<acute>N = n \<and> \<acute>M = m\<rbrace> \<acute>N :==\<^sub>s \<acute>N + 1;;\<^sub>s \<acute>M :==\<^sub>s \<acute>M + 1
...
lemma (in vars)
  "\<Gamma>\<turnstile> \<lbrace>\<acute>N = n \<and> \<acute>M = m\<rbrace>
       LEMMA\<^sub>s foo_lemma
             \<acute>N :==\<^sub>s \<acute>N + 1;;\<^sub>s \<acute>M :==\<^sub>s \<acute>M + 1
       END;;\<^sub>s
       \<acute>N :==\<^sub>s \<acute>N + 1
...
lemma (in vars)
  "\<Gamma>\<turnstile> \<lbrace>\<acute>N = n \<and> \<acute>M = m\<rbrace>
              \<acute>N :==\<^sub>s \<acute>N + 1;;\<^sub>s \<acute>M :==\<^sub>s \<acute>M + 1;;\<^sub>s
              \<acute>N :==\<^sub>s \<acute>N + 1;;\<^sub>s \<acute>M :==\<^sub>s \<acute>M + 1
      \<lbrace>\<acute>N = n + 2 \<and> \<acute>M = m + 2\<rbrace>"
  apply (hoare_rule anno=
          "LEMMA\<^sub>s foo_lemma
...
procedures\<^sub>s foo (N::nat|M::nat)
  "\<acute>M :==\<^sub>s \<acute>M
   \<comment> \<open>think of body with guards instead\<close>"

  foo_spec: "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^sub>t (P \<sigma>) \<acute>M :== PROC\<^sub>s foo(\<acute>N) (Q \<sigma>)"
  foo_modifies: "\<forall>\<sigma>. \<Gamma>\<turnstile>\<^bsub>/UNIV\<^esub> {\<sigma>} \<acute>M :== PROC\<^sub>s foo(\<acute>N)
...
lemma (in append'_impl) append'_modifies:
  shows
   "\<forall>\<sigma>. \<Gamma>\<turnstile> {\<sigma>} \<acute>p :== PROC\<^sub>s append'(\<acute>p,\<acute>q)
        {t. t may_only_modify_globals \<sigma> in [next]}"
  apply (hoare_rule HoarePartial.ProcRec1)
  apply (vcg spec=modifies)
  done
...
Consider for example the simple procedure body @{term "\<acute>p :==\<^sub>s NULL"} for a procedure
\<open>init\<close>.
\<close>

procedures\<^sub>s init (|p) =
  "\<acute>p:==\<^sub>s Null"
...
@{term "\<acute>p :==\<^sub>s NULL"} in the first place by adding a typing annotation:
\<close>

procedures\<^sub>s init' (|p) =
  "\<acute>p:==\<^sub>sNull::(('a globals_list_scheme, 'b) list_vars_scheme, char list, 'c) com"
