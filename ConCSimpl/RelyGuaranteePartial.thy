...
theory RelyGuaranteePartial imports CSimpl.XVcgCon begin


lemma conseq_no_aux:
  "\<lbrakk>\<Gamma>,\<Theta>'\<turnstile>\<^bsub>/F\<^esub> C sat [p',R',G, q',a']; \<Theta>' \<subseteq> \<Theta>;
    \<forall>s. s \<in> p \<longrightarrow> (s\<in>p' \<and> (q' \<subseteq> q) \<and> (a' \<subseteq> a) \<and> (G' \<subseteq> G) \<and> (R\<subseteq>R') )\<rbrakk>
  \<Longrightarrow>
  \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p,R,G,q,a]"
  by (rule LocalRG_HoareDef.conseq) auto

lemma conseq_exploit_pre:
             "\<lbrakk>\<forall>s \<in> p. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [{s} \<inter> p,R,G,q,a]\<rbrakk>
              \<Longrightarrow>
              \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p,R,G,q,a]"
  apply (rule lrghoare.Conseq)
  apply clarify
  by (meson IntI order_refl singletonI)
 

lemma conseq:"\<lbrakk>\<forall>Z Zr. \<Gamma>,\<Theta>'\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)]; \<Theta>' \<subseteq> \<Theta> ;
              \<forall>s. s \<in> p \<longrightarrow> (\<exists>Z Zr. s\<in>p' Z \<and> (q' Z \<subseteq> q) \<and> (a' Z \<subseteq> a) \<and> (G' Zr \<subseteq> G) \<and> (R \<subseteq> R' Zr))\<rbrakk>
              \<Longrightarrow>
               \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p, R, G, q,a]"
  by (rule lrghoare.Conseq) (meson order_refl)

lemma Lem: "\<lbrakk>\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)];
             p \<subseteq> {s. \<exists>Z Zr. s\<in>p' Z \<and> (q' Z \<subseteq> q) \<and> (a' Z \<subseteq> a) \<and> (G' Zr \<subseteq> G) \<and> (R \<subseteq> R' Zr)}\<rbrakk>
             \<Longrightarrow>
             \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (lem x C) sat [p, R, G, q,a]"
...
lemma LemAnno:
assumes conseq:  "p \<subseteq> {s. \<exists>Z Zr. s\<in>p' Z \<and> 
                     (\<forall>t. t \<in> q' Z \<longrightarrow> t \<in> q) \<and> (\<forall>t. t \<in> a' Z \<longrightarrow> t \<in> a) \<and>
                     (\<forall>t. t \<in> G' Zr \<longrightarrow> t \<in> G) \<and> (\<forall>t. t \<in> R \<longrightarrow> t \<in> R' Zr)}"
assumes lem: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)]"
shows "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (lem x C) sat [p, R, G, q,a]"
...
assumes conseq:  "p \<subseteq> {s. \<exists>Z Zr. s\<in>p' Z \<and> 
                     ...
                     (\<forall>t. t \<in> G' Zr \<longrightarrow> t \<in> G) \<and> (\<forall>t. t \<in> R \<longrightarrow> t \<in> R' Zr)}"
assumes lem: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),{}]"
shows "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (lem x C) sat [p, R, G, q,{}]"
  apply (rule Lem [OF lem])
  using conseq
  by blast

lemma TrivPost: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)]
                 \<Longrightarrow>
                 \<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), UNIV,UNIV]"
  apply (rule allI)
apply (rule allI)
apply (erule conseq)
apply auto
...
lemma TrivPostNoAbr: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),{}]
                 \<Longrightarrow>
                 \<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), UNIV,{}]"
  apply (rule allI)
apply (rule allI)
apply (erule conseq)
apply auto
...
lemma TrivGua: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)]
                 \<Longrightarrow>
                 \<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), UNIV, (q' Z),(a' Z)]"
  apply (rule allI)
apply (rule allI)
apply (erule conseq)
apply auto
...
lemma conseq_under_new_pre:"\<lbrakk>\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p',R',G', q',a'];
        \<forall>s \<in> p. s \<in> p' \<and> q' \<subseteq> q \<and> a' \<subseteq> a \<and> R \<subseteq> R' \<and> G' \<subseteq> G\<rbrakk>
\<Longrightarrow> \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p,R,G, q,a]"
apply (rule conseq)
    apply (rule allI)
apply (rule allI)
apply assumption
...
lemma conseq_Kleymann:"\<lbrakk>\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [(p' Z), (R' Zr), (G' Zr), (q' Z),(a' Z)];
              \<forall>s \<in> p. (\<exists>Z Zr. s\<in>p' Z \<and> (q' Z \<subseteq> q) \<and> (a' Z \<subseteq> a) \<and> (G' Zr \<subseteq> G) \<and> (R \<subseteq> R' Zr))\<rbrakk>
              \<Longrightarrow>
             \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> C sat [p,R,G, q,a]"
  apply (rule conseq) by auto

thm DynCom hoarep.DynCom

lemma DynComConseq:
  assumes a0:"fst ` p \<subseteq> {s. \<exists>p' q' a' R' G'.  
                  \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C s) sat [p',R',G', q',a'] \<and> 
                  p \<subseteq> p' \<and> q' \<subseteq> q \<and> a' \<subseteq> a \<and> G' \<subseteq> G \<and> R \<subseteq> R'}"  and
          a1:"(Sta p R) \<and> (Sta q R) \<and> (Sta a R) \<and> (\<forall>s. (Normal s, Normal s) \<in> G)"
        shows "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> DynCom C sat [p,R,G, q,a]"
proof-
  have f1:"\<forall>s\<in>(fst ` p). \<exists>p' q' a' R' G'.  
                  \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C s) sat [p',R',G', q',a'] \<and> 
                  p \<subseteq> p' \<and> q' \<subseteq> q \<and> a' \<subseteq> a \<and> G' \<subseteq> G \<and> R \<subseteq> R'" using a0 by auto
  then have "\<forall>s\<in>(fst ` p). \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C s) sat [p,R,G, q,a]"
  proof-
    { fix s
      assume "s\<in>fst ` p"
      then obtain p' q' a' R' G' where "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C s) sat [p',R',G', q',a'] \<and> 
                  p \<subseteq> p' \<and> q' \<subseteq> q \<and> a' \<subseteq> a \<and> G' \<subseteq> G \<and> R \<subseteq> R'" 
        using f1 by auto
      then have "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C s) sat [p,R,G, q,a]" using conseq_under_new_pre
        by (meson LocalRG_HoareDef.conseqPrePost order_refl)
    } thus ?thesis by auto
  qed 
  then show ?thesis using a1 
    by (simp add: lrghoare.DynCom)
qed

lemma SpecAnno: 
 assumes consequence: "p \<subseteq> {s. (\<exists>Z Zr. s\<in>p' Z \<and> (q' Z \<subseteq> q) \<and> (a' Z \<subseteq> a) \<and> (G' Zr \<subseteq> G) \<and> (R \<subseteq> R' Zr))}"
 assumes spec: "\<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C Z) sat [p' Z,R' Zr,G' Zr, q' Z,a' Z]"
 assumes bdy_constant:  "\<forall>Z. C Z = C undefined"
 shows   "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (specAnno C p' R' G' q' a') sat [p,R,G, q,a]"
proof -
  from spec bdy_constant
  have "\<forall>Z Zr.  \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C undefined) sat [p' Z,R' Zr,G' Zr, q' Z,a' Z]"
    by metis
...
lemma SpecAnno': 
 "\<lbrakk>p \<subseteq> {s.  \<exists> Z Zr. s\<in>p' Z \<and> 
            (\<forall>t. t \<in> q' Z \<longrightarrow>  t \<in> q) \<and> (\<forall>t. t \<in> a' Z \<longrightarrow> t \<in>  a) \<and>  
            (\<forall>t. t \<in> G' Zr \<longrightarrow> t \<in>  G) \<and>  (\<forall>t. t \<in>  R \<longrightarrow> t \<in> R' Zr) };
   \<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C Z) sat [p' Z,R' Zr,G' Zr, q' Z,a' Z];
   \<forall>Z. C Z = C undefined
  \<rbrakk> \<Longrightarrow>          
    \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (specAnno C p' R' G' q' a') sat [p,R,G, q,a]"
...
lemma SpecAnnoNoAbrupt: 
 "\<lbrakk>p \<subseteq> {s.  \<exists> Z Zr. s\<in>p' Z \<and> 
            (\<forall>t. t \<in> q' Z \<longrightarrow>  t \<in> q) \<and>   
            (\<forall>t. t \<in> G' Zr \<longrightarrow> t \<in>  G) \<and>  (\<forall>t. t \<in>  R \<longrightarrow> t \<in> R' Zr) };
   \<forall>Z Zr. \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (C Z) sat [p' Z,R' Zr ,G' Zr, q' Z,{}];
   \<forall>Z. C Z = C undefined
  \<rbrakk> \<Longrightarrow>          
    \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (specAnno C p' R' G' q'  (\<lambda>s. {})) sat [p,R,G, q,a]"
apply (rule SpecAnno')
apply auto
done


lemma Skip: "(\<forall>s. (Normal s, Normal s) \<in> G) \<Longrightarrow> 
             Sta q R \<Longrightarrow> p \<subseteq> q \<Longrightarrow> \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> Skip sat [p,R,G, q,a]"
  by (rule LocalRG_HoareDef.Skip [THEN LocalRG_HoareDef.conseqPre],simp)

lemma Basic: "Sta p R \<Longrightarrow> Sta q R \<Longrightarrow> 
              (\<forall>s t. s\<in>p \<and> ((fst t)=f (fst s)) \<and> (snd s =  snd t) \<longrightarrow> (Normal s,Normal t) \<in> G) \<Longrightarrow>
              p \<subseteq> {s. (f (fst s),snd s) \<in> q} \<Longrightarrow>  \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (Basic f e) sat [p,R,G, q,a]"  
  by (auto intro:  lrghoare.Basic )

lemma BasicCond: 
  "\<lbrakk>p \<subseteq> {s. (b (fst s) \<longrightarrow> (f (fst s),snd s)\<in>q) \<and> (\<not> b (fst s) \<longrightarrow> (g (fst s),snd s)\<in>q)}\<rbrakk> \<Longrightarrow>
   Sta p R \<Longrightarrow> Sta q R \<Longrightarrow> 
   (\<forall>s t. s\<in>p \<and> ((fst t)=(\<lambda>s. if b s then f s else g s) (fst s)) \<and> (snd s =  snd t) \<longrightarrow> 
      (Normal s,Normal t) \<in> G) \<Longrightarrow>
   \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (Basic (\<lambda>s. if b s then f s else g s) e) sat [p,R,G, q,a]"
  apply (rule Basic)
  apply auto
  done
