...
theory SmallStepCon imports "EmbSimpl.SmallStep" "LanguageCon"                             
...
type_synonym ('s,'p,'f,'e) config = "('s,'p,'f,'e)com  \<times> 's"
...
where
  ...
  Basicc: "\<Gamma>\<turnstile>\<^sub>c(Basic f e,s) \<rightarrow> (Skip,f s)"

| Specc: "(s,t) \<in> r \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Spec r e,s) \<rightarrow> (Skip,t)"
| SpecStuckc: "\<forall>t. (s,t) \<notin> r \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Spec r e,s) \<rightarrow> (Stuck,s)"

| Guardc: "s\<in>g \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Guard f g c, s) \<rightarrow> (c,s)"
  
| GuardFaultc: "s\<notin>g \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Guard f g c,s) \<rightarrow> (Fault f,s)"
...
| SeqThrowc: "\<Gamma>\<turnstile>\<^sub>c(Seq Throw c\<^sub>2, s) \<rightarrow> (Throw, s)"
| SeqFaultc: "\<Gamma>\<turnstile>\<^sub>c(Seq (Fault f) c\<^sub>2, s) \<rightarrow> (Fault f, s)"
| SeqStuckc: "\<Gamma>\<turnstile>\<^sub>c(Seq Stuck c\<^sub>2, s) \<rightarrow> (Stuck, s)"

| CondTruec:  "s\<in>b \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Cond b c\<^sub>1 c\<^sub>2,s) \<rightarrow> (c\<^sub>1,s)"
| CondFalsec: "s\<notin>b \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c(Cond b c\<^sub>1 c\<^sub>2,s) \<rightarrow> (c\<^sub>2,s)"

| WhileTruec: "\<lbrakk>s\<in>b\<rbrakk> 
              \<Longrightarrow> 
              \<Gamma>\<turnstile>\<^sub>c(While b c,s) \<rightarrow> (Seq c (While b c),s)"

| WhileFalsec: "\<lbrakk>s\<notin>b\<rbrakk> 
               \<Longrightarrow> 
               \<Gamma>\<turnstile>\<^sub>c(While b c,s) \<rightarrow> (Skip,s)"
...
| Awaitc:  "\<lbrakk>s\<in>b; \<Gamma>1=\<Gamma>\<^sub>\<not>\<^sub>a ; \<Gamma>1\<turnstile>\<langle>ca1,Normal s\<rangle> \<Rightarrow> Normal t\<rbrakk> \<Longrightarrow> 
            \<Gamma>\<turnstile>\<^sub>c(Await b ca1 e, s) \<rightarrow> (Skip,t)"

| AwaitFaultc:  "\<lbrakk>s\<in>b; \<Gamma>1=\<Gamma>\<^sub>\<not>\<^sub>a ; \<Gamma>1\<turnstile>\<langle>ca1,Normal s\<rangle> \<Rightarrow> Semantic.Fault f\<rbrakk> \<Longrightarrow> 
            \<Gamma>\<turnstile>\<^sub>c(Await b ca1 e, s) \<rightarrow> (Fault f, s)"
...
| AwaitStuckc:  "\<lbrakk>s\<in>b; \<Gamma>1=\<Gamma>\<^sub>\<not>\<^sub>a ; \<Gamma>1\<turnstile>\<langle>ca1,Normal s\<rangle> \<Rightarrow> Semantic.Stuck\<rbrakk> \<Longrightarrow> 
            \<Gamma>\<turnstile>\<^sub>c(Await b ca1 e, s) \<rightarrow> (Stuck, s)"
...
| AwaitAbruptc: "\<lbrakk>s\<in>b; \<Gamma>1=\<Gamma>\<^sub>\<not>\<^sub>a ; \<Gamma>1\<turnstile>\<langle>ca1,Normal s\<rangle> \<Rightarrow> Abrupt t\<rbrakk> \<Longrightarrow> 
                 \<Gamma>\<turnstile>\<^sub>c(Await b ca1 e,s) \<rightarrow> (Throw,t)"

| Callc: "\<lbrakk>\<Gamma> p = Some bdy ; bdy\<noteq>Call p\<rbrakk> \<Longrightarrow>
         \<Gamma>\<turnstile>\<^sub>c(Call p,s) \<rightarrow> (bdy,s)"

| CallUndefinedc: "\<Gamma> p=None \<Longrightarrow>
         \<Gamma>\<turnstile>\<^sub>c(Call p,s) \<rightarrow> (Stuck,s)"

| DynComc: "\<Gamma>\<turnstile>\<^sub>c(DynCom c,s) \<rightarrow> (c s,s)"
...
| CatchThrowc: "\<Gamma>\<turnstile>\<^sub>c(Catch Throw c\<^sub>2, s) \<rightarrow> (c\<^sub>2, s)"
| CatchSkipc: "\<Gamma>\<turnstile>\<^sub>c(Catch Skip c\<^sub>2,s) \<rightarrow> (Skip,s)"
| CatchFaultc: "\<Gamma>\<turnstile>\<^sub>c(Catch (Fault f) c\<^sub>2,s) \<rightarrow> (Fault f,s)"
| CatchStuckc: "\<Gamma>\<turnstile>\<^sub>c(Catch Stuck c\<^sub>2,s) \<rightarrow> (Stuck,s)"
...
WhileTruec WhileFalsec Awaitc  AwaitFaultc AwaitStuckc AwaitAbruptc Callc CallUndefinedc DynComc Catchc CatchThrowc CatchSkipc
  CatchFaultc CatchStuckc, induct set]
...
inductive_cases stepc_elim_cases [cases set]:
 ...
 "\<Gamma>\<turnstile>\<^sub>c(Fault f,s) \<rightarrow> u"
 "\<Gamma>\<turnstile>\<^sub>c(Stuck,s) \<rightarrow> u"

inductive_cases stepc_not_normal_elim_cases:
 "\<Gamma>\<turnstile>\<^sub>c(Call p, s) \<rightarrow> (p',s')"


inductive_cases stepc_stuck:
"\<Gamma>\<turnstile>\<^sub>c(c, s) \<rightarrow> (Stuck,s')"

inductive_cases stepc_Throw:
"\<Gamma>\<turnstile>\<^sub>c(c, s) \<rightarrow> (Throw,s')"

inductive_cases stepc_Fault:
"\<Gamma>\<turnstile>\<^sub>c(c, s) \<rightarrow> (Fault f,s')"
...
"\<Gamma>\<turnstile>\<^sub>c(Seq c1 c2,s) \<rightarrow> (Throw,s')" 


inductive_cases stepc_elim_cases_Seq_Fault:
"\<Gamma>\<turnstile>\<^sub>c(Seq c1 c2,s) \<rightarrow> (Fault f,s')"

inductive_cases stepc_elim_cases_Seq_Stuck:
"\<Gamma>\<turnstile>\<^sub>c(Seq c1 c2,s) \<rightarrow> (Stuck,s')"
...
inductive_cases stepc_elim_cases_Catch_fault:
"\<Gamma>\<turnstile>\<^sub>c(Catch c1 c2,s) \<rightarrow> (Fault f,s')" 

inductive_cases stepc_elim_cases_Catch_Stuck:
"\<Gamma>\<turnstile>\<^sub>c(Catch c1 c2,s) \<rightarrow> (Stuck,s')" 
...
"\<Gamma>\<turnstile>\<^sub>c (Await b c e,  s) \<rightarrow> (Skip,t)"

inductive_cases stepc_elim_cases_Await_Throw:
"\<Gamma>\<turnstile>\<^sub>c (Await b c e,  s) \<rightarrow> (Throw,t)"

inductive_cases stepc_elim_cases_Await_Fault:
"\<Gamma>\<turnstile>\<^sub>c (Await b c e,  s) \<rightarrow> (Fault f,t)"

inductive_cases stepc_elim_cases_Await_Stuck:
"\<Gamma>\<turnstile>\<^sub>c (Await b c e,  s) \<rightarrow> (Stuck,t)"
...
"final cfg \<equiv> (fst cfg=Skip \<or> fst cfg=Throw \<or> fst cfg = Stuck \<or> (\<exists>f. fst cfg = Fault f))"

definition final_valid::"('s,'p,'f,'e) config \<Rightarrow> bool" where
"final_valid cfg \<equiv> (fst cfg=Skip \<or> fst cfg=Throw)"
...
lemma 
   assumes step_a: "\<Gamma>\<turnstile>\<^sub>c(Await b c e, s) \<rightarrow> (Skip,u)"
   shows step_await_skip_step_c:"(\<Gamma>\<^sub>\<not>\<^sub>a)\<turnstile>(c, Normal s) \<rightarrow>\<^sup>* (sequential Skip, Normal u)" 
  using step_a
  by (metis exec_impl_steps_Normal sequential.simps(1) stepc_elim_cases_Await_Skip)

lemma 
   assumes step_a: "\<Gamma>\<turnstile>\<^sub>c(Await b c e, s) \<rightarrow> (Fault f,u)"
   shows step_await_fault_step_c:"(\<Gamma>\<^sub>\<not>\<^sub>a)\<turnstile>(c, Normal s) \<rightarrow>\<^sup>* (sequential Skip, Semantic.Fault f) \<and> s = u" 
  using step_a
  by (metis exec_impl_steps_Fault sequential.simps(1) stepc_elim_cases_Await_Fault)

lemma 
   assumes step_a: "\<Gamma>\<turnstile>\<^sub>c(Await b c e, s) \<rightarrow> (Stuck,u)"
   shows step_await_stuck_step_c:"(\<Gamma>\<^sub>\<not>\<^sub>a)\<turnstile>(c, Normal s) \<rightarrow>\<^sup>* (sequential Skip, Semantic.Stuck) \<and> s = u" 
  using step_a        
  by (metis exec_impl_steps_Stuck sequential.simps(1) stepc_elim_cases_Await_Stuck)

lemma 
   assumes step_a: "\<Gamma>\<turnstile>\<^sub>c(Await b c e, s) \<rightarrow> (Throw,u)"
   shows step_await_abrupt_step_c:"(\<Gamma>\<^sub>\<not>\<^sub>a)\<turnstile>(c, Normal s) \<rightarrow>\<^sup>* (sequential Throw, Normal u)" 
  using step_a
  by (metis exec_impl_steps_Normal_Abrupt sequential.simps(10) stepc_elim_cases_Await_Throw)


lemma 
   assumes (* exec: "\<Gamma>\<turnstile>\<langle>c,Normal s\<rangle> \<Rightarrow> t" and
           b: "s \<in> b" and *)
           step_a: "\<Gamma>\<turnstile>\<^sub>c(Await b c e, s) \<rightarrow> u"
...
next
  case (2 t)
  thus "final u" by (simp add: final_def)
next 
  case 3 thus "final u"  by (simp add: final_def)
next
  case (4 t) thus "final u" by (simp add: final_def)
qed

lemma skip_no_tran:
  assumes a0: "\<Gamma>\<turnstile>\<^sub>c (Skip, s) \<rightarrow> (c\<^sub>1', s')"
  shows "P"
  by (meson assms stepc_elim_cases(1))

lemma throw_no_tran: assumes a0: "\<Gamma>\<turnstile>\<^sub>c (Throw, s) \<rightarrow> (c\<^sub>1', s')"
  shows "P"
  by (meson assms stepc_elim_cases(11))

lemma fault_no_tran: assumes a0: "\<Gamma>\<turnstile>\<^sub>c (Fault f, s) \<rightarrow> (c\<^sub>1', s')"
  shows "P"
  by (meson assms stepc_elim_cases(13))

lemma stuck_no_tran: assumes a0: "\<Gamma>\<turnstile>\<^sub>c (Stuck, s) \<rightarrow> (c\<^sub>1', s')"
  shows "P"
  by (meson assms stepc_elim_cases(14))
...
lemma steps_Fault: "redex c = Fault f \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c (c, s) \<rightarrow>\<^sup>* (Fault f, s)"
proof(induct c)
  case (Seq c1 c2)
  then show ?case
    by (metis (no_types, hide_lams) SeqFaultc SmallStepCon.SeqSteps 
        SmallStepCon.redex.simps(4) rtranclp.rtrancl_into_rtrancl)
next
  case (Catch c1 c2)
  then show ?case
    by (metis (no_types, hide_lams) CatchFaultc SmallStepCon.CatchSteps 
           SmallStepCon.redex.simps(11) rtranclp.rtrancl_into_rtrancl)
qed(auto)

lemma steps_Stuck: "redex c = Stuck \<Longrightarrow> \<Gamma>\<turnstile>\<^sub>c (c, s) \<rightarrow>\<^sup>* (Stuck, s)"
proof(induct c)
  case (Seq c1 c2)
  then show ?case
    by (metis (no_types, hide_lams) SeqStuckc SmallStepCon.SeqSteps 
        SmallStepCon.redex.simps(4) rtranclp.rtrancl_into_rtrancl)
next
  case (Catch c1 c2)
  then show ?case
    by (metis (no_types, hide_lams) CatchStuckc SmallStepCon.CatchSteps 
           SmallStepCon.redex.simps(11) rtranclp.rtrancl_into_rtrancl)
qed(auto)
(*
...
lemma step_spec_skip_normal_normal:
  assumes a0:"\<Gamma>\<turnstile>\<^sub>c (c,s)  \<rightarrow> (Skip,s')" and
          a1:"c=Spec r e"
        shows "(s,s')\<in>r"
  using stepc_elim_cases(4)[OF a0[simplified a1]] by auto

lemma step_spec_skip_stuck:
  assumes a0:"\<Gamma>\<turnstile>\<^sub>c (c,s)  \<rightarrow> (Stuck,s')" and
          a1:"c=Spec r e"
        shows "\<forall>s'. (s,s')\<notin>r"
  using stepc_elim_cases(4)[OF a0[simplified a1]] by fast
...
assumes a0: "P = Seq p1 p2" and
        a1: "\<Gamma>\<turnstile>\<^sub>c (p1, s) \<rightarrow> (p1, s)"
shows "\<Gamma>\<turnstile>\<^sub>c (P, s) \<rightarrow> (P, s)"
...
lemma call_f_step_not_s_eq_t_false:
  assumes 
     ...
     a1:"(redex P = Call fn \<and> \<Gamma> fn = Some bdy \<and> ~(s=t)) \<or>
...
lemma step_change_p_or_eq_Ns: 
    ...
    shows  "\<not>(P=Q)"
using step
  by (induct P arbitrary: Q s s') (fastforce elim: stepc_elim_cases)+
