...
theory VcgCon 
  imports  "LocalRG_HoareDef"           
             StateSpaceProduct "HOL-Statespace.StateSpaceLocale" Generalise
HeapList 
...
print_translation \<open>
  let
    fun spec_tr' ctxt ((coll as Const _)$
          ...
          let
            fun selector (Const (c, T)) = HoareCon.is_state_var c
              | selector (Const (@{syntax_const "_free"}, _) $ (Free (c, T))) =
                  HoareCon.is_state_var c
              | selector _ = false;
          in
