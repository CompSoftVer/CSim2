...
syntax (output)
  ...
  "_AssignH" :: "'b => 'b => ('a,'p,'f,'e) com" ("(2*_ :==/ _)" [30, 30] 23)
...
definition lvar_nondet_init ::
   "('c \<times> 'd \<Rightarrow> 'a) \<Rightarrow> (('a \<Rightarrow> 'a) \<Rightarrow> ('c \<times> 'd \<Rightarrow>'c \<times> 'd)) \<Rightarrow> ('c \<times> 'd, 'x, 'e,'f) com" where
  "lvar_nondet_init accessor upd \<equiv> Spec {(s, t). \<exists>v. t = (upd (\<lambda>_. v)) s} None"
...
definition
  asm_spec :: "'a itself \<Rightarrow> ('g \<Rightarrow> 'b) \<Rightarrow> bool \<Rightarrow> string
    \<Rightarrow> (addr \<Rightarrow> ('g \<times> 's) \<Rightarrow> ('g \<times> 's))
    \<Rightarrow> (('g \<times> 's) \<Rightarrow> addr list)
    \<Rightarrow> (('g \<times> 's) \<times> ('g \<times> 's)) set"
where
...
lemma asm_spec_enabled:
  ...
  apply (auto simp add: asm_semantics_enabled asm_spec_def upd_globals_def globals_def split_beta)  
  by (metis all_not_in_conv asm_semantics_enabled surj_pair) 
...
lemmas state_eqE = arg_cong[where f="\<lambda>s. (fst s, snd s)", elim_format]
