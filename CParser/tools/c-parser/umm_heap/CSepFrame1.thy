...
instance prod :: (heap_state_type,type) heap_state_type
  apply intro_classes
  by (auto simp add: upd_globals_def globals_def)
...
definition exec_p::"[('g\<times>'l,'p,'e,'f) body,('g\<times>'l,'p,'e,'f) com,('g\<times>'l)\<times>('l list), 
                      ('g\<times>'l,'p,'e,'f) com,('g\<times>'l)\<times>('l list)] 
                    \<Rightarrow> bool" ("_\<turnstile>\<^sub>p \<langle>_,_\<rangle> \<Rightarrow> \<langle>_,_\<rangle>"  [60,20,98,98] 89)
  where 
"exec_p \<Gamma> C s C' s'\<equiv> \<exists>c. (\<Gamma>,c)\<in> cp \<Gamma> C s \<and> C' = fst(last c) \<and> s' = snd(last c)"

definition
  exec_fatal :: "('g\<times>'l,'p,'f,'e)com  \<Rightarrow> ('g\<times>'l,'p,'f,'e) body \<Rightarrow> ('g\<times>'l)\<times>('l list) \<Rightarrow> bool"
where
  "exec_fatal C \<Gamma> s \<equiv> \<exists>s'. (\<exists>f. \<Gamma>\<turnstile>\<^sub>p \<langle>C,s\<rangle> \<Rightarrow> \<langle>Fault f, s'\<rangle>) \<or>  \<Gamma>\<turnstile>\<^sub>p \<langle>C,s\<rangle> \<Rightarrow> \<langle>Stuck, s'\<rangle>"

definition
  exec_fatal_seq :: "('s,'b,'c) Language.com \<Rightarrow> ('s,'b,'c) Semantic.body \<Rightarrow> 's \<Rightarrow> bool"
where
  "exec_fatal_seq C \<Gamma> s \<equiv> (\<exists>f. \<Gamma> \<turnstile> \<langle>C,Normal s\<rangle> \<Rightarrow> xstate.Fault f) \<or>
      (\<Gamma> \<turnstile> \<langle>C,Normal s\<rangle> \<Rightarrow> xstate.Stuck)"
...
definition
  restrict_safe_OK :: "((('g::heap_state_type'\<times>'l)\<times>('l list))) \<Rightarrow> ('g\<times>'l)\<times>('l list) \<Rightarrow> 
       s_addr set \<Rightarrow> (('g\<times>'l),'b,'c,'e) com \<Rightarrow> (('g\<times>'l),'b,'c,'e) com \<Rightarrow> (('g\<times>'l),'b,'c,'e) body \<Rightarrow> bool"
where
  "restrict_safe_OK s t X C C' \<Gamma> \<equiv>
      ((C'=Skip \<or> C' = Throw) \<and> \<Gamma> \<turnstile>\<^sub>p \<langle>C, restrict_htd s X\<rangle> \<Rightarrow> \<langle>C',  (restrict_htd t X)\<rangle>) \<and>       
...
definition
  restrict_safe :: "('g::heap_state_type'\<times>'l)\<times>('l list) \<Rightarrow> ('g\<times>'l)\<times>('l list) \<Rightarrow>
       (('g\<times>'l),'b,'c,'e) com \<Rightarrow> (('g\<times>'l),'b,'c,'e) com \<Rightarrow>  (('g\<times>'l),'b,'c,'e) body \<Rightarrow> bool"
where
  "restrict_safe s t C C' \<Gamma> \<equiv> \<forall>X. restrict_safe_OK s t  X C C' \<Gamma>  \<or> exec_fatal C \<Gamma> (restrict_htd s X)"
...
definition
  mem_safe :: "(('g::heap_state_type'\<times>'l),'b,'c,'e) com \<Rightarrow>
               (('g\<times>'l),'b,'c,'e) body \<Rightarrow> bool"
where
  "mem_safe C \<Gamma> \<equiv> \<forall>s t C'. \<Gamma> \<turnstile>\<^sub>p \<langle>C, s\<rangle> \<Rightarrow> \<langle>C', t\<rangle> \<longrightarrow>  restrict_safe s t C C' \<Gamma>"
...
primrec intra_safe :: "(('g::heap_state_type'\<times>'l),'b,'c,'e) com \<Rightarrow>  bool"
where
  "intra_safe Skip = True"
| "intra_safe (Basic f e)  = (comm_restrict_safe UNIV f \<and>
      point_eq_mod_safe UNIV f (\<lambda>s. lift_state (hst_mem s,hst_htd s)))"
| "intra_safe (Spec r e) = (\<forall>\<Gamma>. mem_safe (Spec r e) (\<Gamma> :: (('g\<times>'l),'b,'c,'e) body))"
...
| "intra_safe Stuck = False"
| "intra_safe (Fault f) = False"

(* lemma noawaits_same_exec:
  assumes 
   a0:"\<Gamma>\<^sub>\<not>\<^sub>a \<turnstile> \<langle>c', Normal (fst s)\<rangle> \<Rightarrow> t" and a1:"c' = sequential C\<^sub>p" and
   a0':"noawaits C\<^sub>p" and a2:"t\<noteq>Semantic.Stuck"
  shows "\<exists>t'. (t = Normal t')  \<longrightarrow> \<Gamma> \<turnstile>\<^sub>p \<langle>C\<^sub>p, s\<rangle> \<Rightarrow> \<langle>Skip, t'\<rangle>"
  using a0 a0' a1 a2                 
...
lemma mem_safe_D:
  "\<lbrakk> \<Gamma> \<turnstile>\<^sub>p \<langle>C, s\<rangle> \<Rightarrow> \<langle>C',t\<rangle>; mem_safe C \<Gamma>; C' = Skip \<or> C' = Throw;
      \<not> exec_fatal C \<Gamma> (restrict_htd s X) \<rbrakk> \<Longrightarrow>
      \<exists>C'. (C'=Skip \<or> C'=Throw) \<and> (\<Gamma> \<turnstile>\<^sub>p \<langle>C,( (restrict_htd s X))\<rangle> \<Rightarrow> \<langle>C', (restrict_htd t X)\<rangle> \<and>
       point_eq_mod (lift_state (hst_mem t,hst_htd t))
          (lift_state (hst_mem s,hst_htd s)) X)"
 unfolding mem_safe_def restrict_safe_def restrict_safe_OK_def by fast     
...
lemma mem_safe_seq_AbruptD:
  "\<lbrakk> \<Gamma> \<turnstile> \<langle>C,Normal s\<rangle> \<Rightarrow> Abrupt t; mem_safe_seq C \<Gamma>;
      ...
      (\<Gamma> \<turnstile> \<langle>C,(Normal (restrict_htd s X))\<rangle> \<Rightarrow> Abrupt (restrict_htd t X) \<and>
       point_eq_mod (lift_state (hst_mem t,hst_htd t))
          (lift_state (hst_mem s,hst_htd s)) X)"
  unfolding  mem_safe_seq_def restrict_safe_seq_def restrict_safe_OK_seq_def
  by (force simp: mem_safe_seq_def restrict_safe_seq_def restrict_safe_OK_seq_def)

lemma mem_safe_FaultD:
  "\<lbrakk> \<Gamma> \<turnstile>\<^sub>p \<langle>C, s\<rangle> \<Rightarrow> \<langle>Fault f,t\<rangle>; mem_safe C \<Gamma> \<rbrakk> \<Longrightarrow>
      exec_fatal C \<Gamma> (restrict_htd s X)"  
  unfolding mem_safe_def restrict_safe_def restrict_safe_OK_def
  by fastforce

lemma mem_safe_StuckD:
  "\<lbrakk>\<Gamma> \<turnstile>\<^sub>p \<langle>C, s\<rangle> \<Rightarrow> \<langle>Stuck,t\<rangle>; mem_safe C \<Gamma> \<rbrakk> \<Longrightarrow>
      exec_fatal C \<Gamma> (restrict_htd s X)"
  unfolding mem_safe_def restrict_safe_def restrict_safe_OK_def
  by fastforce

lemma mem_safe_seq_FaultD:
  "\<lbrakk> \<Gamma> \<turnstile> \<langle>C,Normal s\<rangle> \<Rightarrow> Semantic.Fault f; mem_safe_seq C \<Gamma> \<rbrakk> \<Longrightarrow>
      exec_fatal_seq C \<Gamma> (restrict_htd s X)"
  by (force simp: mem_safe_seq_def restrict_safe_seq_def)

lemma mem_safe_seq_StuckD:
  "\<lbrakk> \<Gamma> \<turnstile> \<langle>C,Normal s\<rangle> \<Rightarrow> Semantic.Stuck; mem_safe_seq C \<Gamma> \<rbrakk> \<Longrightarrow>
      exec_fatal_seq C \<Gamma> (restrict_htd s X)"
  by (force simp: mem_safe_seq_def restrict_safe_seq_def)
...
lemmas exec_other_intros = exec.intros(1-3) exec.intros(5-16) exec.intros(18-19) exec.intros(20-)
...
(* lemma exec_fatal_Seq:
  ...
  by (fastforce simp: exec_fatal_def intro: exec_other_intros)
